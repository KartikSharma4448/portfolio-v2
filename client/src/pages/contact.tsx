...
export default function Contact() {
  ...
  const contactMutation = useMutation({
    mutationFn: async (data: InsertContactMessage) => {
      // Static build: no backend. Open the user's mail client with the
      // message pre-filled so submissions still reach the inbox.
      const subject = encodeURIComponent(`Portfolio enquiry from ${data.name}`);
      const body = encodeURIComponent(
        `Name: ${data.name}\nEmail: ${data.email}\n\n${data.message}`
      );
      window.location.href = `mailto:kartikuma9261@gmail.com?subject=${subject}&body=${body}`;
      return { ok: true };
    },
    onSuccess: () => {
      setIsSubmitted(true);
      ...
      toast({
        ...
        description: "Your message is ready to send. Hit send in your mail app!",
      });
    },
    onError: () => {
      toast({
        title: "Error",
        description: "Couldn't open your email app. Please email me directly.",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <div className="min-h-screen pt-28 pb-20">
      <SEO
        ...
        description="Get in touch with Kartik Sharma for freelance projects, internships, and collaborations. Full Stack Developer based in Jaipur, India. Usually responds within 24 hours."
        keywords={pageKeywords.contact}
        url="https://kartiksharma.site/contact"
        schema={{
